...
\begin{abstract}
Germany faces dual structural pressures: rapid demographic aging driven by the retirement of the baby-boom cohorts and increasing heterogeneity across successive migrant and refugee cohorts. This study introduces an actuarially grounded, dynamically coupled microsimulation framework modeling wealth accumulation, statutory public pension accrual (\textit{SGB VI, \S\S~64--70}), and means-tested social safety net reliance (\textit{SGB XII, Chapter 4}) for five population segments from 2025 through 2070. All monetary variables and projections are expressed in constant 2026 euros. Calibrated against empirical moments from the Federal Statistical Office (Destatis 16. BVB), Deutsche Bundesbank (PHF Wave 4 / DWA 2024), Deutsche Rentenversicherung (DRV Rentenatlas 2024/2025), and IAB-BAMF-SOEP surveys ($N = 50,000$), our econometric estimations identify a conditional wage disadvantage of $-65.5\%$ ($p < 0.0001$) associated with qualification mismatch, while domestic experience yields positive, concave returns. Under the baseline simulation scenario, projected statutory median pensions at age 67 range from \texteuro2,108/mo for German natives to \texteuro1,552/mo for general economic migrants, \texteuro1,272/mo for 2015/16 refugees, and \texteuro797/mo for 2022+ Ukrainian war refugees. Under baseline permanent settlement assumptions, 52.1\% (sensitivity interval: 44.6\%--58.2\%) of post-2022 Ukrainian refugees are projected to have autonomous retirement incomes below an illustrative socio-cultural subsistence benchmark (\texteuro1,113/mo), requiring an additional monthly savings effort ($S^*$) of \texteuro35--\texteuro220/mo depending on portfolio returns and credential recognition. We provide statutory net reconciliations for the standard pensioner (\textit{Standardrentner}), future pension value indexation equations, and sensitivity analyses across credential recognition, return-migration, and asset returns.
\end{abstract}
...
All monetary variables and lifecycle projections throughout this paper are expressed in \textbf{constant 2026 euros}, deflated using the German Consumer Price Index (CPI) and long-term macro expectations. This paper develops an integrated, calibrated dynamic microsimulation and econometric framework modeling the intertemporal interaction between wage formation, wealth accumulation, statutory pensions, and means-tested social assistance across five population cohorts:
\begin{enumerate}
...
\begin{equation}
    \text{EP}_t = \min\left( \frac{\text{BBG}_t}{\text{DE}_t}, \frac{\min(w_t, \text{BBG}_t)}{\text{DE}_t} \right)
\end{equation}
relative to the preliminary 2026 average reference wage ($\text{DE}_{2026} = \text{\texteuro}51,944$) and the 2026 statutory contribution ceiling ($\text{BBG}_{2026} = \text{\texteuro}101,400$/year, or \texteuro8,450/month), yielding a statutory annual ceiling of $\text{EP}_{\text{max}, 2026} \approx 1.9521$. $\text{AR}_{2026} = \text{\texteuro}42.15$ is the Current Pension Value, $\text{RAF} = 1.0$ is the pension type factor for old-age retirement, and $\text{ZF}$ is the access factor reflecting deductions of $0.3\%$ per month for early retirement or bonuses of $0.5\%$ per month for delayed claiming past age 67 \cite{drv2025rentenatlas}. Childcare credits ($\text{EP}_{\text{child}}$, \S~56 SGB VI) grant up to 3.0 EP per child born after 1991, and \textit{Grundrente} supplements ($\text{EP}_{\text{basic}}$, \S~76g SGB VI) provide targeted point uplifts for workers with at least 33 qualifying contribution years.

\subsection{Future Pension Value Projection to 2070}
To project the evolution of the Current Pension Value ($\text{AR}_t$) through 2070 under demographic aging, we implement the statutory indexation mechanism codified in \S~68 \textit{SGB VI}:
\begin{equation}
    \text{AR}_{t+1} = \text{AR}_t \times \frac{\bar{w}_{t-1}}{\bar{w}_{t-2}} \times \left( \left[ 1 - \alpha \left( \frac{\text{SDR}_{t-1}}{\text{SDR}_{t-2}} - 1 \right) \right] \times \frac{1 - \tau_{t-1}}{1 - \tau_{t-2}} \right)
\end{equation}
where $\bar{w}$ denotes gross wage growth, $\tau$ is the statutory contribution rate, $\text{SDR}_t$ is the system dependency ratio (retirees per contributor) directly coupled to the Destatis 16. BVB projection variants, and $\alpha = 0.25$ is the statutory weighting factor of the Sustainability Factor (\textit{Nachhaltigkeitsfaktor}).

\subsection{Mandatory Deductions \& Standardrentner Reconciliation}
Retirees pay statutory contributions to Health Insurance for Pensioners (\textit{KVdR}) and Long-Term Care Insurance (\textit{PV}). In 2026, the baseline KVdR deduction consists of the 7.3\% statutory base rate plus one-half of the official average \textit{Zusatzbeitrag} (2.9\% / 2 = 1.45\%), totaling **8.75\%**. Long-term care insurance is set at **3.6\%** for pensioners with children and **4.2\%** for childless pensioners. Table~\ref{tab:standardrentner} provides the full component reconciliation for the statutory standard pensioner (\textit{Standardrentner / Eckrente}, 45 EP at age 67).
...
Current Pension Value ($\text{AR}_{2026}$) & \texteuro42.15 & \texteuro42.15 \\
\textbf{Gross Monthly Pension ($R_{\text{gross}}$)} & \textbf{\texteuro1,896.75} & \textbf{\texteuro1,896.75} \\
\midrule
KVdR Statutory Base Contribution (7.30\%) & \texteuro138.46 & \texteuro138.46 \\
KVdR Zusatzbeitrag Share (half of 2.90\% = 1.45\%) & \texteuro27.50 & \texteuro27.50 \\
\textit{Total Health Insurance Deduction (8.75\%)} & \texteuro165.97 & \texteuro165.97 \\
\midrule
Long-Term Care Insurance (PV: 3.60\% vs. 4.20\%) & \texteuro68.28 & \texteuro79.66 \\
\textit{Total Mandatory Social Deductions} & \texteuro234.25 (12.35\%) & \texteuro245.63 (12.95\%) \\
\midrule
\textbf{Net Monthly Pension (before income tax)} & \textbf{\texteuro1,662.50} & \textbf{\texteuro1,651.12} \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
\end{equation}
where \texteuro563 represents the statutory Level 1 standard rate and \texteuro550 represents an illustrative average benchmark for reasonable accommodation and heating costs (\textit{KdU}). The baseline simulation applies a \texteuro10,000 protected liquid financial-asset allowance (\textit{Schonverm{\"o}gen}, \S~90 SGB XII), while separately modeling statutory exemptions for protected owner-occupied housing and exempt pension rights.

\section{Data Architecture, Copula Calibration \& Sample Flow}

\subsection{Parameter Provenance & Sample Flow}
To ensure computational reproducibility without violating federal data privacy laws (\S~16 \textit{BStatG}, \S~75 \textit{SGB X}), we generate a moment-preserving synthetic microdata sandbox ($N = 50,000$). Fig.~\ref{fig:sample_flow} details the sample selection architecture.

\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\columnwidth}{
\textbf{Synthetic Microdata Population ($N = 50,000$)}\\
$\bullet$ Preserves empirical moments from Destatis, Bundesbank, DRV, IAB\\
...
\subsection{Moment Preservation and Goodness-of-Fit}
To preserve multivariate covariance structures, we fit a parametric Vine Copula $C(u_1, \dots, u_d)$ across marginal distributions. Table~\ref{tab:moment_validation} validates the fidelity of the synthetic sandbox against empirical institutional benchmarks.
...
We simulate lifecycle trajectories across $40,500$ working-age individuals until statutory retirement at age 67 under baseline demographic parameters. The Net Replacement Rate (NRR) is formulated as:
\begin{equation}
    \text{NRR} = \frac{\text{Total Net Monthly Retirement Income at 67}}{\text{Pre-Retirement Net Monthly Earnings}} \times 100\%
\end{equation}
...
\toprule
\textbf{Cohort} & \textbf{Pre-Ret Net} & \textbf{Pension} & \textbf{Income} & \textbf{NRR} & \textbf{Subsistence Gap} \\
\midrule
German Native & \texteuro3,000 & \texteuro2,108 & \texteuro2,631 & 87.5\% & 0.3\% \\
General Migrant (1st Gen) & \texteuro2,535 & \texteuro1,552 & \texteuro2,018 & 79.6\% & 13.2\% \\
Ukrainian Migrant (Pre-22) & \texteuro2,575 & \texteuro1,480 & \texteuro1,967 & 76.4\% & 11.7\% \\
General Refugee (2015/16) & \texteuro2,133 & \texteuro1,272 & \texteuro1,596 & 74.8\% & 10.1\% \\
Ukrainian Refugee (2022+) & \texteuro2,092 & \texteuro797 & \texteuro1,061 & 50.7\% & 52.1\% \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
As shown in Table~\ref{tab:sim_results} and Fig.~\ref{fig:income_comp}, Ukrainian war refugees (2022+ arrivals) face a median statutory pension deficit (\texteuro797/mo), leaving 52.1\% (sensitivity interval: 44.6\%--58.2\%) below the illustrative \texteuro1,113 subsistence floor without social assistance top-ups under baseline permanent settlement assumptions.
...
\begin{equation}
    S^* = \frac{\max\left(0, B_{\text{Target}} - [R_{\text{net}} + W_{\text{drawdown}}]\right) \cdot \ddot{a}_{\overline{K}|r_{\text{post}}}}{s_{\overline{N}|r_{\text{pre}}}}
\end{equation}
where $B_{\text{Target}} = \max(\text{\texteuro}1,113, \min(\text{\texteuro}1,800, 0.60 \times w_{\text{net}}))$ represents the standard adequacy benchmark, $\ddot{a}_{\overline{K}|r_{\text{post}}} = \frac{1 - (1 + r_{\text{post}}/12)^{-12K}}{r_{\text{post}}/12}$ is the present value factor of an annuity due over retirement horizon $K = 20$ years, and $s_{\overline{N}|r_{\text{pre}}} = \frac{(1 + r_{\text{pre}}/12)^{12N} - 1}{r_{\text{pre}}/12}$ is the future value accumulation factor over working horizon $N = 67 - \text{age}$. Portfolio returns are expressed as annual real after-fee returns ($r_{\text{pre}} = 4.5\%, r_{\text{post}} = 2.0\%$).
...
\begin{table}[htbp]
\caption{Sensitivity Analysis for Post-2022 Ukrainian Refugee Cohort}
\centering
\begin{adjustbox}{width=\columnwidth}
\begin{tabular}{lcccc}
\toprule
\textbf{Scenario Dimension} & \textbf{Variant} & \textbf{Median Pension} & \textbf{NRR} & \textbf{Req. Savings $S^*$} \\
\midrule
\multirow{3}{*}{Foreign Credential Recognition} & 0\% (Status Quo) & \texteuro797 & 50.7\% & \texteuro180 / mo \\
 & 50\% Fast-Track & \texteuro1,085 & 61.2\% & \texteuro95 / mo \\
 & 100\% Full Match & \texteuro1,390 & 72.5\% & \texteuro35 / mo \\
\midrule
\multirow{3}{*}{Return Migration Trajectory} & 0\% (Full Settlement) & \texteuro797 & 50.7\% & \texteuro180 / mo \\
 & 30\% Partial Return & \texteuro845 & 52.8\% & \texteuro165 / mo \\
 & 60\% High Return & \texteuro910 & 55.4\% & \texteuro140 / mo \\
\midrule
\multirow{3}{*}{Real Equity Portfolio Return} & 2.0\% (Conservative) & \texteuro797 & 48.2\% & \texteuro220 / mo \\
 & 4.5\% (Baseline) & \texteuro797 & 50.7\% & \texteuro180 / mo \\
 & 6.0\% (Optimistic) & \texteuro797 & 54.1\% & \texteuro125 / mo \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
\midrule
German Native & \texteuro2,045 & \texteuro2,178 & 89.4\% & 85.7\% \\
General Migrant & \texteuro1,532 & \texteuro1,570 & 81.8\% & 77.5\% \\
General Refugee & \texteuro1,227 & \texteuro1,310 & 75.2\% & 74.5\% \\
Ukrainian Refugee & \texteuro768 & \texteuro894 & 50.0\% & 52.2\% \\
Ukrainian Migrant & \texteuro1,446 & \texteuro1,516 & 76.7\% & 75.7\% \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
\begin{enumerate}
    ...
    \item \textbf{Foreign Pension Portability}: Potential accrued pension claims in countries of origin (e.g., Ukraine) are omitted due to data limitations and uncertain bilateral portability agreements.
    \item \textbf{Stationary Institutional Rules}: Statutory pension parameters follow official demographic indexation formulas but do not anticipate unannounced legislative major structural reforms.
\end{enumerate}

\section{Policy Recommendations}
\begin{enumerate}
    \item \textbf{Fast-Track Credential Recognition}: Accelerated qualification homologation under the \textit{Berufsqualifikationsfeststellungsgesetz} could substantially reduce the wage disadvantage associated with qualification mismatch, potentially lifting up to 78\% of vulnerable refugee workers out of structural old-age poverty risk under counterfactual full-recognition scenarios.
